...
export const usePokerHandStatusStore = defineStore('poker-hand-status-store', () => {
  ...
  async function loadHandStatus(userHand: IPokerHand = 'Two Pair') {
    try {
      ...
      axios
        // .get(
        //   `https://poker-hand-analytics-git-main-pedroararipes-projects.vercel.app/analysis/hand?player_current_hand=${userHand}`,
        // )
        .get(`http://127.0.0.1:8000/analysis/hand?player_current_hand=${userHand}`)
        .then((r: AxiosResponse<IPlayerHandStatus>) => (handStatus.value = r.data))
    } catch (e) {
      console.error(e)
    } finally {
